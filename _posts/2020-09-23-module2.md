...
### npm audit fix로 취약점을 해결하자

언제부터인가 `npm install`을 하면 아래와 같은 메시지가 나왔다.  
취약점이 발견되었으니 `npm audit fix`를 하라는 뜻이다.
...
이게 무엇인고 하니, `npm install`을 할 때 package의 취약점을 검사한다.  
그리고 `npm audit fix`로 취약점이 발견된 package를 자동으로 update 해준다.
...
`npm audit fix`을 해도 취약점이 다 해결되지 않는 경우가 많다.  
그러면 취약점이 더 줄어들지 않을 때까지 `npm audit fix`를 해준다.  
그래도 취약점이 0이 되지 않으면 `npm audit`으로 뭐가 문제인지 확인한 후 수작업으로 문제를 해결해준다.  
...
위와 같이 확인 후 문제가 되는 `nightwatch`를 update 한다.
...
하지만 계속 업데이트하고 의존성 취약 모듈을 수정해줘도 아래처럼 문구가 이렇게 나올 때도 있다.
